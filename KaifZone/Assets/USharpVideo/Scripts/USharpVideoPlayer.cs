...
namespace UdonSharp.Video
{
    ...
    public class USharpVideoPlayer : UdonSharpBehaviour
    {
        ...
        public override void OnVideoError(VideoError videoError)
        {
            ...
            switch (videoError)
            {
                case VideoError.RateLimited:
                    _statusStr = "Скорость ограничена, попробуйте позже";
                    break;
                case VideoError.PlayerError:
                    _statusStr = "Ошибка видео плеера";
                    break;
                case VideoError.InvalidURL:
                    _statusStr = "Неверная ссылка";
                    break;
                case VideoError.AccessDenied:
                    _statusStr = "Видео заблокировано, включите Untrusted URLs";
                    break;
                default:
                    _statusStr = "Ошибка загрузки видео";
                    break;
            }
            SetStatusText(_statusStr);
            PlayNextVideoFromPlaylist();
        ...
        private void Update()
        {
            ...
            if (_masterOnly && !Networking.IsMaster)
            {
                urlPlaceholderText.text = $"Только мастер {Networking.GetOwner(gameObject).displayName} может вставлять ссылки";
                inputField.readOnly = true;
                lockGraphic.color = redGraphicColor;
            }
            else if (!_masterOnly)
            {
                urlPlaceholderText.text = "Вставьте ссылку на видео... (каждый)";
                inputField.readOnly = false;
                lockGraphic.color = whiteGraphicColor;
            }
            else
            {
                urlPlaceholderText.text = "Вставьте ссылку на видео...";
                inputField.readOnly = false;
